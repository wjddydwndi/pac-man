class Enemy extends GameObj{
	constructor(type,container,x,y,width,height,velX,velY,targetX,targetY,bg,src){
		...
		센서들을 배치한다
		------------------------------------------------------------*/
		this.leftSensor=new Sensor("SENSOR", container, getSensorSize("LEFT",this.x, this.y, this.width, this.height) , 0, 0, 0, 0, "#000","");
		this.rightSensor=new Sensor("SENSOR", container, getSensorSize("RIGHT",this.x, this.y, this.width, this.height) , 0, 0, 0, 0, "#000","");
		this.upSensor=new Sensor("SENSOR", container, getSensorSize("UP",this.x, this.y, this.width, this.height) , 0, 0, 0, 0, "#000","");
		this.downSensor=new Sensor("SENSOR", container, getSensorSize("DOWN",this.x, this.y, this.width, this.height) , 0, 0, 0, 0, "#000","");
	...
	tick(){
			...
			if(obj.type=="BLOCK"){
				...
				for(var a=0;a<this.sensorArray.length;a++){
					...
					if(hitTest(obj , s, this.velX, this.velY)){ //충돌이라면...
						hitCount[a]++;
						//console.log(hitCount , "충돌 , key = ", autoKey);
					}
				}
			}
		}
		...
		---------------------------------------------------------------------------------------------------*/
						...
						if(hitCount[1] > 0 && key==38){//상단에 닿으면
							this.velY=0;
						}
						if(hitCount[2] > 0 && key ==39){//우측에 닿으면
							this.velX=0;
						}
						...
						this.x+=this.velX;
						this.y+=this.velY;*/
			if(hitCount[0] > 0  || hitCount[1] >0){//좌측이 닿으면
				this.velX=-this.velX;
			}
			if(hitCount[2] > 0 || hitCount[3] > 0){//상단이 닿으면 
				this.velY=-this.velY;
			}
